...
async function exportReport() {
    const range = document.getElementById('rangeOptions')?.value || 'all';
    const res = await fetch(`/dashboard/export_report?range=${encodeURIComponent(range)}`);
    if (!res.ok) return alert('Failed to create report');
    const text = await res.text();
    const blob = new Blob([text], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `dashboard-report-${range}.txt`;
...
async function exportCSV() {
    const range = document.getElementById('rangeOptions')?.value || 'all';
    const res = await fetch(`/dashboard/export_csv?range=${encodeURIComponent(range)}`);
...
function generateSummary() {
    const total = document.getElementById('totalPatients')?.textContent || '-';
    const high = document.getElementById('highRiskCases')?.textContent || '-';
    const top = document.getElementById('topPCB')?.textContent || '-';
    const avg = document.getElementById('avgFetalPCB')?.textContent || '-';
