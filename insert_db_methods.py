...
def update_patient_record(patient_id: int, name: str, bdate: str, age: int, gestational_age: int, due_date: str, physician_id: int,
                          data_consent: bool, is_complete: bool, appointment_datetime: str, appointment_purpose: str, features: dict, session: Session):
    ...
    patient.age = age
    patient.gestational_age = gestational_age
    patient.due_date = datetime.strptime(due_date, "%Y-%m-%d").date()
    patient.data_consent = True if data_consent == "on" else False
    patient.is_complete = True if is_complete == "on" else False
    patient.physician_id = physician_id

    session.add(patient)

    appt = session.exec(
        select(Appointment).where(Appointment.patient_id == patient_id).order_by(
            Appointment.datetime.desc()).limit(1)
    ).first()

    if appointment_datetime and appointment_purpose:
        if appt:
            appt.datetime = datetime.strptime(
                appointment_datetime, "%Y-%m-%dT%H:%M")
            appt.purpose = appointment_purpose
            appt.physician_id = physician_id
            session.add(appt)
        else:
            session.add(Appointment(
                purpose=appointment_purpose,
