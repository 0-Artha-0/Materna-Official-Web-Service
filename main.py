...
from models import Account, Patient, Appointment, ModelFeatures
...
@app.get("/dashboard/export_report", response_class=PlainTextResponse)
def export_dashboard_report(range: str = Query("all"), session: Session = Depends(get_session)):
    start, end = dash.get_range_bounds(range)
    report = dash.build_dashboard_report_text(session, start, end)
    return PlainTextResponse(report, media_type="text/plain")


@app.get("/dashboard/export_csv", response_class=PlainTextResponse)
def export_dashboard_csv(range: str = Query("all"), session: Session = Depends(get_session)):
    start, end = dash.get_range_bounds(range)
    csv_data = dash.build_dashboard_csv(session, start, end)
...
def data_clerk_edit_patient(request: Request, patient_id: int, session: Session = Depends(get_session)):
    ...
    patient_data = dbf.get_data_clerk_patient_data(
        session=session, patient_id=patient_id)
    if not patient_data:
...
async def register_new_patient(request: Request, session: Session = Depends(get_session)):
    ...
    if patient_id:
        status = dbi.update_patient_record(
            int(patient_id),
            data["name"],
            data["bdate"],
            int(data["age"]),
            int(data["gestational_age"]),
            data["due_date"],
            int(data["physician_id"]),
            data.get("consent"),
            data.get("is_complete"),
            data.get("appointment_datetime"),
            data.get("appointment_purpose"),
            features,
            session,
        )
        if status != 200:
        ...
        dbi.insert_new_patient(data["name"], data["bdate"], data["age"], data["gestational_age"], data["due_date"], data["physician_id"],
                               data.get("consent"), data.get("is_complete"), data.get("appointment_datetime"), data.get("appointment_purpose"), features, session)

    logger.debug("Patient saved or updated!!")
