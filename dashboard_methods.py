...
def build_dashboard_report_text(session: Session, start: datetime, end: datetime) -> str:
    summary = fetch_summary(session, start, end)
    lines = [
        f"Dashboard report ({start:%Y-%m-%d} to {end:%Y-%m-%d})",
        "",
        f"Total Patients Assessed: {summary['total_patients']}",
        f"High Risk Cases: {summary['high_risk']}",
        f"Top PCB Compound: {summary['top_pcb']}",
        f"Top PCB Average: {summary['top_pcb_avg']}",
        f"Average Fetal PCB: {summary['avg_fetal_pcb']}",
        f"Maternal-Fetal Correlation: {summary['correlation']}",
    ]
    return "\n".join(lines)


def build_dashboard_csv(session: Session, start: datetime, end: datetime) -> str:
    summary = fetch_summary(session, start, end)
    rows = [
        ["Metric", "Value"],
        ["Total Patients Assessed", summary['total_patients']],
        ["High Risk Cases", summary['high_risk']],
        ["Top PCB Compound", summary['top_pcb']],
        ["Top PCB Average", summary['top_pcb_avg']],
        ["Average Fetal PCB", summary['avg_fetal_pcb']],
        ["Maternal-Fetal Correlation", summary['correlation']],
    ]
